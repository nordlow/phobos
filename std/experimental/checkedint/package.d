...
$(MREF std,experimental,_checkedint, throws), $(MREF std,experimental,_checkedint, asserts), or $(MREF std,experimental,_checkedint, noex), depending on which error signalling policy you want
to use. (See below.)
...
$(OL
    ...
    $(LI `checkedint` can't slow down code where it's not used! If you really need more speed, try switching to
        $(LREF DebugInt) for the hottest code in your program (like inner loops) before giving up on `checkedint`
        entirely.)
)
...
$(UL
    $(LI Always use $(LREF smartOp.mulPow2), $(LREF smartOp.divPow2), and $(LREF smartOp.modPow2) whenever they can
        naturally express your intent - they're faster than a regular `/`, `%`, or `pow()`.)
...
// debug /////////////////////////////////////////////////
    ...
    If performance is your only motivation for using `DebugInt` rather than $(LREF SmartInt), consider limiting
